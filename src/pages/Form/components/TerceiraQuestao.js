...
const TerceiraQuestao = ({ juntarRespostas, dispatch, respostas}) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
    ...
    defaultValues: {
      pontuacaoExperienciaTrabalho: 0,
      pontuacaoMembroComunidade: 0
    }
  })
    ...
    <ScrollView w='95%'>
        ...
        <VStack space={6} marginBottom={8}>
          <FormControl isInvalid={errors.experienciaDeTrabalho}>
            <FormControl.Label>
              Você tem alguma experiência de trabalho na família, em empresa ou instituição ligada à sua comunidade?
            </FormControl.Label>
            <Controller
          ...
          <FormControl isInvalid={errors.acaoComunitaria}>
            <FormControl.Label>
              Você participa e/ou participou em sua comunidade de algum tipo de ação comunitária junto a igreja, associações, clubes de mães, sindicatos etc.?
            </FormControl.Label>
            <Controller
          ...
          <VStack space={4}>
            <Controller
              ...
              render={({ field: { onChange, onBlur, value } }) => (
                <Box>
                  <FormControl.Label>
                    Sua família ou você participa ou já participaram de algum tipo de organização? Qual?
                  </FormControl.Label>
                  <Radio.Group
                    onChange={(value) => {
                      onChange(value)
                      setSelectParticipacaoFamilia(value)
                    }}
                    value={value || ''}
                  >
          ...
          <FormControl isInvalid={errors.pontuacaoExperienciaTrabalho}>
            <FormControl.Label flexDirection='colum'>
              a) Possui experiência de trabalho em instituição ligada a povos ou comunidades tradicionais.
